...
class MultiThreadedExecutor:
    ...
    def fill(self, data_func, in_data, out_shape = None):
        def _fill(data_func, in_data, out_data, first, last, stride = 20):
            for i in range(first,last,stride):
                out_data[i:min(i+stride,last)] = data_func(in_data[i:min(i+stride,last)]) #[data_func(d) for d in in_data[first:last]]
